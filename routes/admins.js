...
router.get("/:idAdmin", (req, res) => {
  ...
  connection.query("SELECT * FROM admin WHERE id = ?", [idAdmin], (err, results) => {
    if (err) {
      res.status(500).send(`Erreur lors de la récupération d'un administrateur`);
    }
    if (results.length === 0) {
...
router.post("/", (req, res) => {
  ...
  connection.query("INSERT INTO admin SET ?", [idAdmin], (err, results) => {
    if (err) {
      res.status(500).send(`Erreur lors de la création d'un administrateurs`);
    } else {
      res.sendStatus(200);
    }
  });
...
router.put("/:idAdmin", (req, res) => {
  ...
  connection.query("UPDATE admin SET ? WHERE id = ?", [formData, idAdmin], (err) => {
    if (err) {
      res.status(500).send(`Erreur lors de la création d'un administrateur`);
    } else {
      res.sendStatus(200);
    }
  });
