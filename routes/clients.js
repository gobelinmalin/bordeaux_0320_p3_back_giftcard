...
router.get('/:idClient/favorites', (req, res) => {
    ...
    connection.query('SELECT * FROM favorite_product AS f JOIN product AS p ON f.id_product = p.id WHERE id_client = ?', idClient, (err, result) => {
        if (err) {
...
router.get('/:idClient/favorites/shops', (req, res) => {
    const {idClient} = req.params;
    connection.query('SELECT * FROM favorite_shop AS f JOIN shop AS s ON f.id_shop = s.id WHERE id_client = ?', [idClient], (err, result) => {
        if (err) {
